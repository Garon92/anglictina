...
function SlowPlayButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-2 mx-auto px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium text-sm transition-all active:scale-95"
    >
          ...
          {(['all', 'dictation', 'comprehension', 'gapfill'] as const).map((t) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                filterType === t ? 'bg-primary-500 text-white' : 'bg-slate-100 text-slate-600'
              }`}
