...
import { VOCAB_EXAMPLES_2 } from './vocabExamples2';
import { VOCAB_EXAMPLES_BY_ID_1 } from './vocabExamplesById1';
import { VOCAB_EXAMPLES_BY_ID_2, VOCAB_EXAMPLES_BY_ID_2_COLLISIONS } from './vocabExamplesById2';
import { VOCAB_EXAMPLES_BY_ID_3 } from './vocabExamplesById3';
import { VOCAB_EXAMPLES_BY_ID_4 } from './vocabExamplesById4';
...
const RICH_ENTRIES: VocabWord[] = [
  ...
  w('do', 'dělat / udělat', 'verb', 1, ['daily', 'work'], 'What do you do?', 'Co děláš?'),
  w('go', 'jít / jet', 'verb', 1, ['travel', 'daily'], 'We go to school every day.', 'Chodíme do školy každý den.'),
  w('come', 'přijít / přijet', 'verb', 1, ['daily'], 'Come here, please.', 'Pojď sem, prosím.'),
  w('take', 'vzít / brát', 'verb', 1, ['daily'], 'Take your bag.', 'Vezmi si tašku.'),
  w('make', 'dělat / udělat / vyrobit', 'verb', 1, ['daily', 'work'], 'She makes breakfast every morning.', 'Každé ráno připravuje snídani.'),
  w('see', 'vidět', 'verb', 1, ['daily'], 'I can see the mountains.', 'Vidím hory.'),
  w('know', 'vědět / znát', 'verb', 1, ['daily'], 'I know the answer.', 'Znám odpověď.'),
  w('think', 'myslet (si) / přemýšlet', 'verb', 1, ['daily'], 'I think it is a good idea.', 'Myslím, že je to dobrý nápad.'),
  w('get', 'dostat / získat', 'verb', 1, ['daily'], 'Did you get my message?', 'Dostal jsi mou zprávu?'),
  w('want', 'chtít', 'verb', 1, ['daily', 'shopping'], 'I want a glass of water.', 'Chci sklenici vody.'),
  w('say', 'říct / říkat', 'verb', 1, ['daily'], 'What did you say?', 'Co jsi řekl?'),
  w('use', 'používat / použít', 'verb', 1, ['daily', 'media'], 'I use my phone every day.', 'Používám telefon každý den.'),
  w('give', 'dát / dávat', 'verb', 1, ['daily'], 'Give me the book, please.', 'Dej mi tu knihu, prosím.'),
  w('find', 'najít', 'verb', 1, ['daily'], 'I cannot find my keys.', 'Nemůžu najít klíče.'),
  w('tell', 'říct / vyprávět', 'verb', 1, ['daily'], 'Tell me about your day.', 'Pověz mi, jaký jsi měl den.'),
  w('ask', 'zeptat se / požádat', 'verb', 1, ['daily', 'education'], 'Can I ask a question?', 'Můžu se na něco zeptat?'),
  w('work', 'pracovat', 'verb', 1, ['work', 'daily'], 'I work in an office.', 'Pracuji v kanceláři.'),
  w('try', 'zkusit / snažit se', 'verb', 1, ['daily', 'food'], 'Try this cake!', 'Ochutnej tenhle dort!'),
  ...
  w('leave', 'odejít / opustit / nechat', 'verb', 1, ['travel', 'daily'], 'I leave home at eight.', 'Odcházím z domu v osm.'),
  ...
  w('let', 'nechat / dovolit', 'verb', 1, ['daily'], 'Let me help you.', 'Dovol, abych ti pomohl.'),
  w('begin', 'začít / začínat', 'verb', 1, ['daily'], 'The movie begins at eight.', 'Film začíná v osm.'),
  w('show', 'ukázat / ukazovat', 'verb', 1, ['daily'], 'Can you show me the way?', 'Můžeš mi ukázat cestu?'),
  ...
  w('run', 'běžet / běhat', 'verb', 1, ['sports', 'daily'], 'She runs every morning.', 'Každé ráno běhá.'),
  w('move', 'hýbat se / přestěhovat se', 'verb', 1, ['housing', 'daily'], 'We moved to a new house.', 'Přestěhovali jsme se do nového domu.'),
  ...
  w('bring', 'přinést / přivést', 'verb', 1, ['daily'], 'Bring your friends!', 'Přiveď kamarády!'),
  w('happen', 'stát se', 'verb', 1, ['daily'], 'What happened yesterday?', 'Co se včera stalo?'),
  w('write', 'psát / napsat', 'verb', 1, ['education', 'daily'], 'Write your name here.', 'Napiš sem své jméno.'),
  w('learn', 'učit se / naučit se', 'verb', 1, ['education'], 'I am learning English.', 'Učím se anglicky.'),
  w('read', 'číst', 'verb', 1, ['education', 'freetime'], 'I read books every day.', 'Každý den čtu knihy.'),
  w('buy', 'koupit / kupovat', 'verb', 1, ['shopping'], 'I want to buy a new phone.', 'Chci si koupit nový telefon.'),
  ...
  w('sleep', 'spát', 'verb', 2, ['health', 'daily'], 'I sleep eight hours a night.', 'Každou noc spím osm hodin.'),
  w('speak', 'mluvit', 'verb', 1, ['education', 'daily'], 'Do you speak English?', 'Mluvíš anglicky?'),
  w('walk', 'chodit (pěšky) / procházet se', 'verb', 1, ['travel', 'sports'], 'I walk to school.', 'Chodím pěšky do školy.'),
  w('drive', 'řídit', 'verb', 1, ['travel'], 'Can you drive a car?', 'Umíš řídit auto?'),
  w('pay', 'platit / zaplatit', 'verb', 1, ['shopping'], 'Can I pay by card?', 'Mohu platit kartou?'),
  w('send', 'poslat / posílat', 'verb', 1, ['media', 'daily'], 'Send me a message.', 'Pošli mi zprávu.'),
  w('open', 'otevřít / otvírat', 'verb', 1, ['daily'], 'Open the window, please.', 'Otevři okno, prosím.'),
  w('close', 'zavřít / zavírat', 'verb', 1, ['daily'], 'Close the door.', 'Zavři dveře.'),
  w('watch', 'sledovat / dívat se', 'verb', 1, ['freetime', 'media'], 'I watch TV in the evening.', 'Večer se dívám na televizi.'),
  w('wait', 'čekat / počkat', 'verb', 1, ['daily'], 'Wait for me!', 'Počkej na mě!'),
  ...
  w('like', 'mít rád / líbit se', 'verb', 1, ['daily'], 'I like chocolate.', 'Mám rád čokoládu.'),
  ...
  w('man', 'muž / člověk', 'noun', 1, ['family', 'daily'], 'The man is tall.', 'Ten muž je vysoký.'),
  w('woman', 'žena', 'noun', 1, ['family', 'daily'], 'The woman is a doctor.', 'Ta žena je lékařka.'),
  ...
  w('home', 'domov', 'noun', 1, ['housing', 'family'], "There's no place like home.", 'Všude dobře, doma nejlépe.'),
  ...
  w('money', 'peníze', 'noun', 1, ['shopping', 'work'], "I don't have enough money.", 'Nemám dost peněz.'),
  w('head', 'hlava', 'noun', 1, ['health'], 'My head hurts.', 'Bolí mě hlava.'),
  w('car', 'auto', 'noun', 1, ['travel'], 'He drives a red car.', 'Jezdí červeným autem.'),
  w('room', 'pokoj / místnost', 'noun', 1, ['housing'], 'My room is small but cozy.', 'Můj pokoj je malý, ale útulný.'),
  w('water', 'voda', 'noun', 1, ['food', 'nature'], 'Can I have some water?', 'Můžu dostat trochu vody?'),
  ...
  w('street', 'ulice', 'noun', 2, ['travel', 'housing'], 'Our school is on a busy street.', 'Naše škola je v rušné ulici.'),
  w('story', 'příběh', 'noun', 1, ['culture', 'freetime'], 'Tell me a story.', 'Vyprávěj mi příběh.'),

  // ─── Education & School ────────────────────────────────────────────
  w('teacher', 'učitel / učitelka', 'noun', 2, ['education'], 'Our teacher is very nice.', 'Náš učitel je velmi milý.'),
  w('lesson', 'hodina (vyučovací) / lekce', 'noun', 2, ['education'], 'We have six lessons a day.', 'Máme šest hodin denně.'),
  ...
  w('passport', 'cestovní pas', 'noun', 2, ['travel'], "Don't forget your passport.", 'Nezapomeň si cestovní pas.'),
  ...
  w('restaurant', 'restaurace', 'noun', 2, ['food', 'travel'], "Let's go to a restaurant.", 'Pojďme do restaurace.'),
  ...
  w('rain', 'déšť', 'noun', 2, ['weather', 'nature'], 'We walked home in the rain.', 'Šli jsme domů v dešti.'),
  ...
  w('bad', 'špatný', 'adjective', 1, ['daily'], 'I had a bad dream.', 'Měl jsem zlý sen.'),
  ...
  w('dangerous', 'nebezpečný', 'adjective', 2, ['daily'], 'Swimming here is dangerous.', 'Koupat se tady je nebezpečné.'),
  ...
  w('always', 'vždy / vždycky', 'adverb', 1, ['daily'], 'I always wake up early.', 'Vždy vstávám brzy.'),
  ...
  w('usually', 'obvykle', 'adverb', 2, ['daily'], 'I usually have tea for breakfast.', 'K snídani obvykle piju čaj.'),
  w('already', 'už / již', 'adverb', 1, ['daily'], 'I have already finished.', 'Už mám hotovo.'),
  ...
  w('together', 'spolu / dohromady', 'adverb', 1, ['family', 'daily'], "Let's go together.", 'Pojďme spolu.'),
  ...
  w('therefore', 'proto / tudíž', 'adverb', 2, ['education'], 'He studied hard, and therefore he passed.', 'Pilně se učil, a proto zkoušku udělal.'),
  ...
  w('immediately', 'okamžitě / ihned', 'adverb', 2, ['daily'], 'Call the doctor immediately.', 'Okamžitě zavolejte lékaře.'),
  w('unfortunately', 'bohužel', 'adverb', 3, ['daily'], 'Unfortunately, the shop is closed.', 'Obchod je bohužel zavřený.'),
  w('recently', 'nedávno', 'adverb', 2, ['daily'], 'I recently started a new job.', 'Nedávno jsem nastoupil do nové práce.'),
  ...
  // ─── Months missing from the NGSL list ────────────────────────────
  w('March', 'březen', 'noun', 1, ['weather', 'daily'], 'My birthday is in March.', 'Mám narozeniny v březnu.'),
  w('May', 'květen', 'noun', 1, ['weather', 'daily'], 'The maturita exams start in May.', 'Maturitní zkoušky začínají v květnu.'),
];
...
function compactToVocab(
  ...
  return entries.map(([en, cs, pos], i) => {
    // Chunk 2 (919 words from rank 701) overlaps the ranks of chunk 3 (from 1501). Historic progress
    // is stored under the plain id, which always resolved to the chunk-2 word, so the later
    // duplicate gets a distinct "-b" id.
    let id = `ngsl-${startRank + i}`;
    if (usedIds.has(id)) id = `${id}-b`;
    usedIds.add(id);
    return { id, en, cs, example: '', exampleCs: '', band, topics: [], partOfSpeech: pos as Pos };
  });
}

/** Original positional id (without the "-b" suffix) — examples by id were keyed by it. */
function baseId(id: string): string {
  return id.endsWith('-b') ? id.slice(0, -2) : id;
}

const BRITISH: Record<string, string> = {
  color: 'colour', behavior: 'behaviour', neighbor: 'neighbour', favorite: 'favourite', center: 'centre',
  theater: 'theatre', labor: 'labour', honor: 'honour', humor: 'humour', harbor: 'harbour',
};

/** Does `sentence` use the headword (allowing simple inflections)? */
function usesWord(sentence: string, word: string): boolean {
  const s = sentence.toLowerCase();
  const w = word.toLowerCase();
  if (s.includes(w)) return true;
  const stem = w.replace(/(e|y)$/, '');
  return stem.length >= 3 && s.includes(stem);
}

type Example = { example: string; exampleCs: string };

const EXTRA_EXAMPLES: Record<string, Example> = {
  'ngsl-279': { example: 'Their love for animals is obvious.', exampleCs: 'Jejich láska ke zvířatům je zřejmá.' },
};

function findExample(w: VocabWord, ambiguousIds: Set<string>): Example | undefined {
  const key = w.en.toLowerCase();
  const id = baseId(w.id);
  if (EXTRA_EXAMPLES[w.id]) return EXTRA_EXAMPLES[w.id];
  const byId = [VOCAB_EXAMPLES_BY_ID_1[id], VOCAB_EXAMPLES_BY_ID_2[id], VOCAB_EXAMPLES_BY_ID_3[id], VOCAB_EXAMPLES_BY_ID_4[id]];
  const collision = VOCAB_EXAMPLES_BY_ID_2_COLLISIONS[id]?.[key];
  if (collision) return collision;
  // Ranks shared by two words: only accept an example that really contains this headword.
  const trusted = ambiguousIds.has(id) ? byId.filter((c) => c && usesWord(c.example, w.en)) : byId;
  const fromId = trusted.find(Boolean);
  if (fromId) return fromId;
  return VOCAB_EXAMPLES[key] ?? VOCAB_EXAMPLES_2[key] ?? (BRITISH[key] ? VOCAB_EXAMPLES[BRITISH[key]] ?? VOCAB_EXAMPLES_2[BRITISH[key]] : undefined);
}

// A rich entry replaces the NGSL card with the same headword AND part of speech.
const richKeys = new Set(RICH_ENTRIES.map((w) => `${w.en.toLowerCase()}|${w.partOfSpeech}`));
const usedIds = new Set<string>();
let ambiguousCache: Set<string> | null = null;
function ambiguous(all: VocabWord[]): Set<string> {
  if (!ambiguousCache) ambiguousCache = new Set(all.filter((w) => w.id.endsWith('-b')).map((w) => baseId(w.id)));
  return ambiguousCache;
}

const ngslCompact = [
  ...compactToVocab(NGSL_CHUNK1, 1, 1, usedIds),
  ...compactToVocab(NGSL_CHUNK2, 2, 701, usedIds),
  ...compactToVocab(NGSL_CHUNK3, 3, 1501, usedIds),
  ...compactToVocab(NGSL_CHUNK4, 3, 2101, usedIds),
]
  .filter((w) => !richKeys.has(`${w.en.toLowerCase()}|${w.partOfSpeech}`))
  .map((w, _i, all) => {
    const ex = findExample(w, ambiguous(all));
    return ex ? { ...w, example: ex.example, exampleCs: ex.exampleCs } : w;
  });
